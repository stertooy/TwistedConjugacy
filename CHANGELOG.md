...
### Changed
  ...
  - Improved the efficiency of `CoincidenceReidemeisterSpectrum` for
    non-abelian groups.
...
### Changed
  ...
  - Applying `Size` to an infinite Reidemeister class will now return
    `infinity` instead of running indefinitely, if the range is
    nilpotent-by-finite.
